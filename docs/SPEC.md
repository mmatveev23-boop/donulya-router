...
status: draft v0.1 · ждёт ревью владельца по открытым вопросам
related: regulation-цвета-сделок-и-зоны-клоузера.md, spec-виджет-amocrm-данные-и-контроль.md, decisions.md
---
...
**Реакция клоузера:**
- Звонит в течение 5 мин → Sipuni webhook `call_initiated` → роутер сбрасывает таймер, переводит `Assignment` в `ENGAGED`
- Не позвонил 5 мин → таймер истекает → роутер переназначает следующему по той же формуле, статус первого `Assignment` → `TIMEOUT` (без штрафа), создаётся новый `Assignment`
...
--   "active_statuses": [84657777, 84657778, ...]
-- }
...
CREATE TABLE rules (
  id              SERIAL PRIMARY KEY,
  department_id   INTEGER REFERENCES departments(id),
  kind            TEXT NOT NULL,                 -- 'schedule'|'telephony_block'|'capacity'|'light_zone'|'custom_filter'
  config          JSONB NOT NULL,
  enabled         BOOLEAN DEFAULT true,
  order_idx       INTEGER DEFAULT 0,             -- порядок применения фильтров
  created_at      TIMESTAMPTZ DEFAULT NOW()
);
-- Пример: rule{kind='capacity', config={'limit': 40, 'count_pipelines': [10780422]}}
    ...
    for rule in department.rules.where(enabled=True).order_by('order_idx'):
        if rule.kind == 'manual_block':
            ...
            for c in list(candidates):
                if (await get_active_count(c.id, rule.config['count_pipelines'])) >= limit:
                    rejected.append((c, f'capacity {limit} reached'))
                    candidates.remove(c)
...
INSERT INTO rules (department_id, kind, config) VALUES
  ...
  (<hunters_id>, 'capacity', '{"limit": 60, "count_pipeline_ids": [<id_охотников>]}'),
...
| Capacity | 40 | 60 | 80 |
| Стратегия | Round-Robin + светофор | Round-Robin + светофор-охотник | Weighted_workload |
| Метрика зон | % красных сделок | конверсия трипваера 7d | overdue cases |
...
- **Виджет шапки** показывает огненный pill «🔥 Новый лид: Петров К. → Взять» с обратным таймером 5:00 → 0:00. Клик = эквивалент «Я взял» (сбрасывает таймер ENGAGED).
...
- `cf_user_active_count` — текущее число активных сделок (для отображения, не источник истины)
...
- МПП2: «Новый клиент» (триггер), «Думает», «Обещал оплатить», «Оплатил частичку», «Полная продажа» (для capacity 40)
...
### C. Что считать «звонил» для сброса 5-мин таймера ✅ ЗАКРЫТО

**Решение:** факт инициации исходящего звонка (без требования к длительности). Главное — клоузер начал работать с лидом за 5 минут.

Реализация: Sipuni webhook `call_started` от клоузера на номер лида → роутер находит активный `Assignment` и переводит в `ENGAGED`. Если телефония не интегрирована (MVP) — таймер сбрасывается через **кнопку в виджете «Я взял»** + клоузер обязан позвонить (контроль через регламент, а не автоматический).

«Зачёт касания» для светофора сделки — это **другая метрика** (≥90 сек + продвижение в CRM), считается отдельно.
...
### I. Лимит 40 — что считается «активным» ✅ ЗАКРЫТО

**Решение:**
- **Воронка:** только МПП2 (`pipeline_id=10780422`). Партнёрские лиды (donulya-partnerka) **НЕ учитываются**.
- **Этапы:** "Думает", "Обещал оплатить", "Оплатил частичку", "Полная продажа".
- **НЕ учитываются:** "Закрыто и реализовано", "Закрыто и не реализовано", "Пауза", "Неактивный" (любые архивные/паузные статусы).

Реализация: в правиле `kind='capacity'` указываем точные `stage_ids` (требуется уточнение реальных ID из amoCRM при разработке).
...
```json
{
  ...
  "config": {
    "limit": 40,
    "count_pipeline_ids": [10780422],
    "count_stage_ids": [<думает>, <обещал>, <частичка>, <полная>]
  }
}
```
...
### Фаза 1 · MVP «один отдел, один триггер» (2 недели)
...
- [ ] Алгоритм матчинга: schedule + manual_busy + capacity(40) + light_zone(новая формула, новые пороги) + Round-Robin
...
### Фаза 2 · Интеграции телефонии (1 неделя)
- [ ] Sipuni webhook receiver → `user_availability_state`
- [ ] МегаФон (если есть) — то же
- [ ] Контур — sync через ical (worker 1 раз в 5 мин)
